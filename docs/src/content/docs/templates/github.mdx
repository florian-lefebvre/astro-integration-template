...
This template uses Github Actions. Copy it locally by running the following command:

```sh
pnpm dlx giget@latest gh:florian-lefebvre/astro-integration-template/templates/github my-integration
```
...
7. Follow the [the `e18e` publishing guide](https://e18e.dev/docs/publishing.html). The release workflow is properly configured for OIDC publishing and is called `release.yml`
...
5. The package will be published to NPM automatically throuh OIDC.
